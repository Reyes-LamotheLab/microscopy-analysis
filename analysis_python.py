    ...
    for i in range (len(outlines)):
        ...
        if lenght>min :
            ...
            for j in range ( len(data)):
                ...
                x=data[j][col_x]   #CHANGE THIS SO THE COLUMN IS ALIGNED WITH X_(px)
                y=data[j][col_y]    #CHANGE THIS SO THE COLUMN IS ALIGNED WITH Y_(px)
                coord=[x,y]
                ...
                if point_in_poly(x,y,poly): #if particle inside the cell segmentation
                    ...
                    table[z][4]=x
                    table[z][5]=y
                    table[z][6]=data[j][col_int] ##CHANGE THIS SO THE COLUMN IS ALIGNED IntegratedInt
                    table[z][7]=center[0]
                    table[z][8]=center[1]
                    table[z][9]=distance
...
def size_colocalization(Input_chan, other_chan, pixel_dist, table, size):
    ...
    for key1 in Input_chan.keys(): #iterates through all the cells of the image 
        ...
        if len(other_chan[key1])==0:
            print(key1, "here") #if the cell other chanel has no particle return result=0 
            continue         
...
def main1(directory_seg, directory_res, *args): 
    ...
    new_dict = {0.5:0, 1.0:0, 1.5:0, 2:0, 2.5:0, 3:0}   #Here you can change the number of distances you want by deleting or adding the desired pixel distance, 
...
if __name__ == "__main__":
    ...
    directory_seg= input("Enter results folder path: ")
    directory_seg= directory_seg.replace('\\', '/')
    directory_res= input("Enter Segmentations folder path: ")
    directory_res = directory_res.replace('\\', '/')
    if input("Do you want the swarmplot of the Integrated Intensity of cells? Input Y or N:")=="Y":

        folder_integrated=input("Enter IntDensity folder of the results from the Integrated script:")
    
    if input("Do you have two channels or only one? Enter 1 or 2:")=="1":
        main1(directory_seg, directory_res, folder_integrated)
    
    else:
        main2(directory_seg, directory_res, folder_integrated)
