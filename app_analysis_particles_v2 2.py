...
def integrated_int_histogram(filename, color):
    ...
    output_file = os.path.join(results_folder_path, f'{filename}_particleInt.png')
...
def coloc_integrated_plot(filename):
    ...
    plt.savefig(str(results_folder_path) + '/' + 'coloc_per_integratedIntensity.png')
...
def table_creation(data, header, outlines, size, min):
    ...
    # the final table with 10 columns and 5000 rows (5000 is arbitrary to not cause error)
    table = np.zeros((5000, 10))
    count = 0
...
def main2(directory_seg, directory_res, folder_integrated, parameters, selected_folder_path):
    ...
    for filename in sorted(os.listdir(directory_seg)):
        ...
        if not filename.endswith(".DS_Store"):
            ...
            table_1, counts_1, one, two, three, four, five, six, coloc, distance, cellnum=table_creation(fiji_1,header, outlines,pixel, too_small)
